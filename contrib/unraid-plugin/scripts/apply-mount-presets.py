...
def parse_mount_line(line: str) -> tuple[str, str, str] | None:
    ...
    dst = parts[1].rstrip("/") or "/"
    return parts[0], dst, mode
...
def merge_mounts(
    ...
    by_dest = {norm_dest(dst): (src, norm_dest(dst), mode) for src, dst, mode in existing}
    for src, dst, mode in desired:
        key = norm_dest(dst)
        by_dest[key] = (src, key, mode)
...
def sanitize_mounts(mounts: list[tuple[str, str, str]]) -> list[tuple[str, str, str]]:
    ...
    for src, dst, mode in mounts:
        key = norm_dest(dst)
        if key in DEPRECATED_DESTINATIONS or key.rstrip("/") in {
            d.rstrip("/") for d in DEPRECATED_DESTINATIONS
        }:
            continue
        if src.startswith("/etc/wolf/"):
            continue
        cleaned.append((src, key, mode))
    # Remove anonymous/named Docker volume sources (e.g. lutris:/var/lutris) when
    # a host bind exists for the same destination.
    host_by_dest = {
        norm_dest(dst): (src, dst, mode)
        for src, dst, mode in cleaned
        if src.startswith("/")
    }
    out: list[tuple[str, str, str]] = []
    for src, dst, mode in cleaned:
        key = norm_dest(dst)
        if not src.startswith("/") and key in host_by_dest:
            continue
