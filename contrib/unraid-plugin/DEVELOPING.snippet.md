...
## Clean install with custom GOW images (dev test)

For a full wipe, local Fedora image build, Wolf `config.toml` image override, mount presets, and verification, use `scripts/dev-install-test.sh` on the Unraid server as **root**.

### Prerequisites

- GoW plugin installed and **Compose Manager** present
- GPU + library paths configured in **Settings → Games on Whales** (ROMs, BIOS, Media, Steam, Lutris, Games, …)
- GOW repo on the server, e.g. `git clone` into `/mnt/user/appdata/gow-src` on branch `draft/gow-library-mounts`

### One-shot flow (after first-time UI Install)

```bash
GOW_SRC=/mnt/user/appdata/gow-src \
  bash /boot/config/plugins/gow/scripts/dev-install-test.sh \
    --wipe --build --post-install
```

This will:

1. Run `wipe-full.sh` (removes appdata and Moonlight pairing)
2. Build `gow/<app>-fedora:unraid-test` for es-de, steam, lutris, retroarch, pegasus
3. Patch `runner.image` in `$APPDATA/cfg/config.toml`
4. Run `fix-all.sh` and `repair-esde.sh`
5. Run `verify-session-mounts.sh` and `health-check.sh` (fails if unhealthy)

Complete **Settings → Install** again after `--wipe`, then re-run with `--build --post-install` if you wiped mid-test.

### Split workflow

```bash
# 1. Wipe only
bash /boot/config/plugins/gow/scripts/dev-install-test.sh --wipe

# 2. UI: Install + set libraries

# 3. Build images + apply mounts + verify
GOW_SRC=/mnt/user/appdata/gow-src \
  bash /boot/config/plugins/gow/scripts/dev-install-test.sh --build --post-install
```
...
```bash
APPDATA=/mnt/user/appdata/gow
for title in EmulationStation Steam Lutris Retroarch Pegasus Kodi Plex Emby; do
  echo "=== $title ==="
  grep -A30 "title = \"$title\"" "$APPDATA/cfg/config.toml" | grep -E 'image|mounts'
done
bash /boot/config/plugins/gow/scripts/verify-session-mounts.sh
bash /boot/config/plugins/gow/scripts/health-check.sh
```

Pair Moonlight at `http://<UNRAID_IP>:<WOLF_DEN_PORT>/Clients/Pairing`, then launch each app once and check session logs for mount warnings.
